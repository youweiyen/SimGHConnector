...
namespace SimGH
{
    public class GH_RunSimulation : GH_Component
    {
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            pManager.AddBooleanParameter("Run", "R", "Run Simulation", GH_ParamAccess.item);
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            DA.GetData(1, ref runSim);

            if (runSim)
            { 
                var simulationRunApi = new SimulationRunsApi(simProjectInfo.Configuration);
                var restClient = new RestClient();
                var API_KEY_HEADER = "X-API-KEY";
                var API_KEY = simProjectInfo.ApiKey;
